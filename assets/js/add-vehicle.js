...
$(document).ready(function () {
  ...
  $(".next-step").click(function () {
    let visible = true; // Assume all fields are filled initially

    // Check for empty values in input, select, and radio fields
    $(".step-" + currentStep);
    // const formQuestionArea = $(".step-" + currentStep).find(".form-question-area");
    // if (formQuestionArea.length > 0) {
    //   const checkedRadioButtons = formQuestionArea.find(".yes-no-input:checked");
    //   if (checkedRadioButtons.length === 0) {
    //     formQuestionArea.find(".required-span").text("Select at least one option");
    //     visible = false;
    //   } else {
    //       visible = true;
    //     formQuestionArea.find(".required-span").text("");
    //   }
    // }
  ...
  updateProgressBar = function () {
    var progressPercentage = ((currentStep - 1) / 6) * 100;
...
(function (root, factory) {
    ...
    function loadImage(imageFile) {
      ...
      reader.onload = function () {
        const fragment = document
          .createRange()
          .createContextualFragment(htmlElements.imagePreviewTemplate.innerHTML);
        const imageElement = fragment.querySelector("img");
  
        fragment.querySelector("[data-image-preview-element]").dataset.index =
          imageFiles.length - 1;
  ...
  const releasePopupBtn = document.querySelector('.release-pop-btn')
  const releasePopup = document.querySelector('.release-liability-popup')
...
const signature_text = document.getElementById('e_signature');
const signParagraph = document.getElementById('sign');

// Add event listeners to the input fields
signature_text.addEventListener('input', updateSign);

// Function to update the content of the paragraph
function updateSign() {
    // Combine the values of the first name and last name
    const fullName = `${signature_text.value}`;
